...
unsigned char onD = 0xFF;

// keep track of which output is displayed
unsigned char output = 1;
unsigned char outputD = 1;
    ...
    switch(state) 
    {
        case OUTPUT:
        {
            if(on)
            {
                switch(output) 
                {
                    case 1: shiftWrite(output1, 1); break;
                    case 2: shiftWrite(output2, 1); break;
                    case 3: shiftWrite(output3, 1); break;
                }
            }
            else
            {
            ...
            if(onD)
            {
                switch(outputD)
                {
                    case 1: shiftWrite(output1, 2); break;
                    case 2: shiftWrite(output2, 2); break;
                    case 3: shiftWrite(output3, 2); break;
                }
            }
            else
            {
...
{ 
    ...
    buttonTask.state = BWAIT;
    buttonTask.period = 50;
    buttonTask.elapsedTime = buttonTask.period;
    buttonTask.TickFct = &buttonSM;
    
    dButtonTask.state = DWAIT;
    dButtonTask.period = 50;
    dButtonTask.elapsedTime = dButtonTask.period;
    dButtonTask.TickFct = &dButtonSM;
